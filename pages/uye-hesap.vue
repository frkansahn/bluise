         ...
         <div class="col-11 col-sm-6 col-md-4 m-auto pb-5 pt-5 px-4 shadow-lg">
            ...
            <b-form>
               <b-form-group id="name_group" label="İsim" label-for="name_input" description="">
                  <b-form-input :class="[registerError.name ? 'border-danger':'border-dark']" id="name_input" v-model="register.name" type="text" name="name" placeholder="İsminiz" @click="registerError.name=false" required disabled></b-form-input>
               </b-form-group>

               <b-form-group id="surname_group" label="Soyisim" label-for="surname_input" description="">
                  <b-form-input :class="[registerError.surname ? 'border-danger':'border-dark']" id="surname_input" v-model="register.surname" type="text" name="surname" placeholder="Soyisminiz" @click="registerError.surname=false" required disabled></b-form-input>
               </b-form-group>

               <b-form-group id="tc_kimlik_group" label="TC Kimlik" label-for="tc_kimlik_input" description="">
                  <b-form-input :class="[registerError.tc_kimlik ? 'border-danger':'border-dark']" id="tc_kimlik_input" v-model="register.tc_kimlik" type="number" name="uniqueId" placeholder="Tc Kimlik Numarası" maxlength="11" @click="registerError.tc_kimlik=false" required disabled></b-form-input>
               </b-form-group>
               ...
               <b-form-group id="email_group" label="Email" label-for="email_input" description="">
                  <b-form-input :class="[registerError.email ? 'border-danger':'border-dark']" name="email" id="email_input" v-model="register.email" type="text" placeholder="Emailiniz" @click="registerError.email=false" required></b-form-input>
               </b-form-group>
   ...
   export default 
   {
      ...
      methods:{
         async registerCustomer(){
            ...
            if((_this.register.password == _this.register.again_password) && _this.$checkEmail(_this.register.email) && _this.register.phone.replace(/\D/g, '').length == 11)
            {
               _this.registerOverlay = true;      
               _this.register.phone = Number(_this.register.phone.replace(/\D/g, ''));
               _this.$store.dispatch('postService' , {"data":_this.register , "urlParameter":"/api/v1/customer/update/" + cusId.value , "token":token})
               .then(response => {
      ...
      async created(){
         ...
         if(token)
         {
            _this.$store.dispatch('getService' , {"urlParameter":"/api/v1/customerToken/customer/" + token , "token": ""})
            .then(response => {
               _this.register = response;
               _this.register.again_password = _this.register.password;
