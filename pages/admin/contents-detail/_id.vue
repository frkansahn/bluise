                  ...
                  <b-form-group class="col-md-12" id="description_group" label="Detaylı Açıklama" label-for="description_input" description="">
                     <editor
                        ...
                        :init="{
                           ...
                           resize: 'both',
                           plugins: 'link image media autolink lists advlist media table advtable fullscreen lists directionality emoticons wordcount fullpage preview tabfocus template anchor autoresize advcode',
                           menubar: 'file edit insert view format table tools help',
                           toolbar: 'undo redo | styleselect | fullscreen | forecolor  | bold italic underline italic | alignleft aligncenter alignright alignjustify | outdent indent | link image media| code table | numlist bullist | ltr rtl | emoticons | wordcount | anchor | fullpage | template | preview | grid_insert',
                  ...
                  <div class="col-12 mt-4">
                     <div class="row">
                        <div class="col-12 col-md-6 m-auto">
                           <b-button class="col-12" type="button" @click="updateContents" squared size="lg" variant="primary">Güncelle</b-button>
                        </div>
                     </div>                     
                  </div>
                  ...
                  <div class="col-12 mt-4">
                     <div class="row">
                        <div class="col-10 col-md-4 m-auto">
                           <b-button class="col-12" type="button" @click="deleteContents" squared size="md" variant="danger">Sil</b-button>
                        </div>
                     </div>                     
                  </div>
               </b-form>
   ...
   export default 
   {
      ...
      methods:{
         async deleteContents(){
            var _this = this;
            _this.$bvModal.msgBoxConfirm("İçerik'i silmek istediğinize emin misiniz?",{
               size: 'sm',
               buttonSize: 'sm',
               okVariant: 'success',
               okTitle: 'Evet',
            ...
            .then(value => {
               if(value)
               {
                  _this.$store.dispatch('getService' , {"urlParameter":"/api/v1/contents/delete/" + _this.$route.params.id , "token": _this.token})
                  .then(response => {
                     if(response.success == true)
                     {
                        _this.$bvToast.toast('içerik başarıyla silindi.', {
                           title: `Başarılı..`,
                           variant: 'success',
                           autoHideDelay: 500,
                           solid: true
                     ...
                     {
                        _this.$bvToast.toast('İçerik silinemedi.Sayfayı yenileyip tekrar deneyiniz.', {
                           title: `Başarısız..`,
                           variant: 'danger',
                           autoHideDelay: 500,
                           solid: true
         ...
         updateContents(){
            ...
            if(_this.contents.subject != '' && _this.contents.description != '' && _this.contents.short_description != '' && _this.contents.category_id != '' && _this.controlSeoLink())
            {
               ...
               _this.$store.dispatch('postService' , {"data":_this.contents , "urlParameter":"/api/v1/contents/update/" + _this.$route.params.id , "token": _this.token})
               .then(response => {
         ...
         async controlSeoLink(){
            ...
            {
               _this.$store.dispatch('postService' , {"data":{link: _this.contents.seo_link} , "urlParameter":"/api/v1/contents/controlLink" , "token":_this.token})
               .then(response => {
                  if(response.use)
                  {
         ...
         async deleteImage(){
            ...
            if(_this.token)
            {
               _this.$store.dispatch('postService' , {"data":{"filename":_this.contents.image} , "urlParameter":"/api/v1/image/removeFile" , "token": _this.token})
               .then(response => {
                  if(response)
                  {
                     _this.contents.image = "";
                     _this.contentImage = undefined;
                  }
                  else
                  {
                     _this.contents.image = "";
      ...
      async created(){
         ...
         {
            _this.token = token;
            _this.contents.saved_by = userId;

            _this.$store.dispatch('getService' , {"urlParameter":"/api/v1/contents/ForAdmin/" + _this.$route.params.id , "token": token})
            .then(response => {
               if(response)
