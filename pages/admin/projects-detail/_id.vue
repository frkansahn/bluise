<template>
   <div class="projects-edit" id="editProjects">
      <AdminHeader/>
               ...
               <b-form class="row">
                  ...
                  <b-form-group class="col-md-3" id="category_group" label="Proje Kategori" label-for="category_input" description="">
                     <b-form-select :class="[projectsError.category_id ? 'border-danger':'border-dark']" v-model="projects.category_id" name="projectsCategory" required>
                        ...
                        <b-form-select-option v-for="(NC , index) in projects_categories" :key="`projects_categories${index}`" :value="NC.id">{{NC.category_name}}</b-form-select-option>
                     </b-form-select>
                  </b-form-group>
                  ...
                  <div class="col-md-2 mt-4">
                     <div class="row">
                        <b-button @click="saveImage" class="mt-2 col-12" variant="success" size="md" v-if="!projects.image">Resim Ekle</b-button>
                        <b-button @click="deleteImage" class="mt-2 col-12" variant="danger" size="md" v-else>Sil</b-button>
                     </div>                     
                  </div>
                  <div class="col-md-3 px-4">
                     <img class="mt-3" :src="projects.image" :alt="projects.subject" v-if="projects.image" style="max-height: 60px;">
                  ...
                  <div class="col-12 mt-4">
                     <div class="row">
                        <div class="col-12 col-md-6 m-auto">
                           <b-button class="col-12" type="button" @click="updateProjects" squared size="lg" variant="primary">Güncelle</b-button>
                        </div>
                     </div>                     
                  </div>
                  ...
                  <div class="col-12 mt-4">
                     <div class="row">
                        <div class="col-10 col-md-4 m-auto">
                           <b-button class="col-12" type="button" @click="deleteProjects" squared size="md" variant="danger">Sil</b-button>
                        </div>
                     </div>                     
                  </div>
               </b-form>
   ...
   export default 
   {
      ...
      methods:{
         async deleteProjects(){
            ...
            _this.$bvModal.msgBoxConfirm('Projeyi silmek istediğinize emin misiniz?',{
               size: 'sm',
               buttonSize: 'sm',
               okVariant: 'success',
               okTitle: 'Evet',
            ...
            .then(value => {
               if(value)
               {
                  _this.$store.dispatch('getService' , {"urlParameter":"/api/v1/projects/delete/" + _this.$route.params.id , "token": token})
                  .then(response => {
                     if(response.success == true)
                     {
                        _this.$bvToast.toast('Proje başarıyla silindi.', {
                           title: `Başarılı..`,
                           variant: 'success',
                           autoHideDelay: 500,
                           solid: true
                     ...
                     {
                        _this.$bvToast.toast('Proje silinemedi.Sayfayı yenileyip tekrar deneyiniz.', {
                           title: `Başarısız..`,
                           variant: 'danger',
                           autoHideDelay: 500,
                           solid: true
         ...
         updateProjects(){
            ...
            if(_this.projects.subject != '' && _this.projects.description != '' && _this.projects.short_description != '' && _this.projects.category_id != '')
            {
               ...
               _this.$store.dispatch('postService' , {"data":_this.projects , "urlParameter":"/api/v1/projects/update/" + _this.$route.params.id  , "token": _this.token})
               .then(response => {
         ...
         async deleteImage(){
            ...
            if(_this.token)
            {
               _this.$store.dispatch('postService' , {"data":{"filename":_this.projects.image} , "urlParameter":"/api/v1/image/removeFile" , "token": _this.token})
               .then(response => {
                  if(response)
                  {
                     _this.projects.image = "";
      ...
      async created(){
         ...
         if(token && userId)
         {
            ...
            this.$store.dispatch('getService' , {"urlParameter":"/api/v1/projects/ForAdmin/" + this.$route.params.id , "token": token})
            .then(response => {
